			...
			OR empty($_POST['check_in']) OR empty($_POST['check_out'])) {
				...
				if(date("Y/m/d") > $check_in OR date("Y/m/d") > $check_out) {
					...
					if($check_in > $check_out OR $check_in == date("Y/m/d")){
						echo "<script>alert('pick a date that is not today for check-in and pick a check-in date that less that check-out date');</script>";

					} else {
						$booking = $conn->prepare("INSERT INTO bookings (check_in, check_out, email, phone_number, full_name,
						hotel_name, room_name, status, paymet, user_id) VALUES (:check_in, :check_out, :email, :phone_number, :full_name, 
      ...
      <div class="container">
        <div class="row no-gutters slider-text js-fullheight align-items-center justify-content-start" data-scrollax-parent="true">
          <div class="col-md-7 ftco-animate">
          	<h2 class="subheading">Welcome to 3 Brothers Empire</h2>
